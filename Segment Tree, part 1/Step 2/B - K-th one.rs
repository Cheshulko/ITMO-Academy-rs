...
mod cm_seg_tree {
    ...
    impl SegmentTree<usize> {
        pub fn kth_one(&self, k: usize) -> Option<usize> {
            self.kth_one_recursive(k, 0, 0..self.len)
        }

        fn kth_one_recursive(
            &self,
            k: usize,
            idx: usize,
            element_range: Range<usize>,
            ...
            if left > k {
                self.kth_one_recursive(k, 2 * idx + 1, element_range.start..mid)
            } else {
                self.kth_one_recursive(k - left, 2 * idx + 2, mid..element_range.end)
            }
        }
    }
}
...
fn main() {
    ...
    for _ in 0.._t {
        ...
        let mut tree = cm_seg_tree::SegmentTree::from_vec(&arr, |left, right| left + right);

        for _ in 0..m {
            let t = cin.next::<usize>();

            if t == 1 {
                let i = cin.next::<usize>();
                let cur = tree.query(i..(i + 1)).unwrap();
                tree.update(i, cur ^ 1);
            } else {
                let k = cin.next::<usize>();
                println!("{ind}", ind = tree.kth_one(k).unwrap());
            }
        }
    }
}
