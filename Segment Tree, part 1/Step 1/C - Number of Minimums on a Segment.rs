...
impl MinCnt {
    pub fn new(min: usize, count: usize) -> Self {
        Self { min, count }
    }
}

impl Default for MinCnt {
    fn default() -> Self {
        Self {
            min: usize::MAX,
            count: 0,
...
fn main() {
    ...
    for _ in 0.._t {
        ...
        let mut arr = vec![];
        for i in 0..n {
            arr.push(MinCnt::new(cin.next::<usize>(), 1));
        }

        let mut tree = cm_seg_tree::SegmentTree::from_vec(&arr, |a, b| match a.min.cmp(&b.min) {
            Ordering::Less => a,
            Ordering::Greater => b,
            Ordering::Equal => MinCnt::new(a.min, a.count + b.count),
        });
        ...
        for _ in 0..m {
            ...
            if t == 1 {
                ...
                tree.update(i, MinCnt::new(v, 1));
            } else {
                let (l, r) = (cin.next::<usize>(), cin.next::<usize>());
                let min_cnt = tree.query(l..r).unwrap();
                println!("{min} {cnt}", min = min_cnt.min, cnt = min_cnt.count);
            }
        }
    }
}
