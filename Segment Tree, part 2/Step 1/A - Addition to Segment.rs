...
mod cm_lazy_seg_tree_capture {
    ...
    // [l, r)
    pub struct LazySegmentTree<F, L, T: Default + Copy + Add<Output = T>>
    where
        F: Fn(&T, &T) -> T,
    ...
    impl<F, L, T: Default + Copy + Add<Output = T>> LazySegmentTree<F, L, T>
    where
        F: Fn(&T, &T) -> T,
        L: Fn(&T, &Range<usize>, &T) -> T,
    {
        pub fn from_vec(arr: &[T], merge: F, apply_lazy: L) -> Self {
            let len = arr.len();
...
fn main() {
    ...
    for _ in 0.._t {
        ...
        let mut tree = cm_lazy_seg_tree_capture::LazySegmentTree::from_vec(
            ...
            |&node_val, element_range, &lazy| {
                node_val + (element_range.end - element_range.start) * lazy
            },
        );
        ...
        for _ in 0..m {
            ...
            if t == 1 {
                ...
                let ans = tree.query(i..(i + 1)).unwrap();
                println!("{ans}");
            }
        }
    }
