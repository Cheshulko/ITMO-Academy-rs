...
fn main() {
    ...
    for _ in 0.._t {
        ...
        let mut tree = cm_lazy_seg_tree_capture::LazySegmentTree::from_vec(
            ...
            |&lazy_1, &lazy_2| match (lazy_1, lazy_2) {
                (None, _) => lazy_2,
                (_, None) => unreachable!(),
                (Some(op1), Some(op2)) => match (op1, op2) {
                    (Op::Assign(_), Op::Assign(_)) => lazy_2,
                    (Op::Assign(ass1), Op::Add(add2)) => Some(Op::Assign(ass1 + add2)),
                    (Op::Add(_), Op::Assign(_)) => lazy_2,
                    (Op::Add(add1), Op::Add(add2)) => Some(Op::Add(add1 + add2)),
                },
            },
            |&node_val, range, &lazy| match lazy {
                Some(Op::Add(add)) => node_val + (range.end - range.start) * add,
                Some(Op::Assign(ass)) => (range.end - range.start) * ass,
                None => node_val,
            },
        );
        ...
        for _ in 0..m {
            ...
            match t {
                1 => {
                    ...
                    tree.update(l..r, Some(Op::Assign(v)));
                }
                2 => {
                    let (l, r, v) = (
                        cin.next::<usize>(),
                    ...
                    tree.update(l..r, Some(Op::Add(v)));
                }
                3 => {
                    let (l, r) = (cin.next::<usize>(), cin.next::<usize>());
                    let sum = tree.query(l..r).unwrap();
                    println!("{sum}");
                }
                _ => unreachable!(),
            }
